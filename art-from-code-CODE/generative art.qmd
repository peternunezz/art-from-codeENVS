...
text <- geom_text( #creates text
  mapping = aes(0.7, 3, label = "IM ALIVE!                                                     "), 
  size = 10, 
  colour = "black", 
  fontface = "bold"
) 
...
text2 <- geom_text(
  mapping = aes(0.9, 2, label = "or am i?                                                                                                    "),
  size = 7,
  colour = "black"
)
...
```{r art pieces}
polar_art <- function(seed, n, palette, backround_color = "red") {
...
polar_art(seed = 1, n = 500, palette = c("#DDFFE7", "#98D7C2", "#167D7F", "#29A0B1", "#003060","#272D83"))
...
artcolors <- dat %>%
  mutate(color_variable = sample(c('#078d70', '#98e8c1', '#ffffff', '#7bade2', '#3d1a78' ), nrow(dat), replace = TRUE))  
...
polar_styled_plot(data = dat, palette = artcolors$color_variable) + geom_path()
...
shaded_ridge_art <- function(seed = NULL) {
  ...
  grid.text(
    label = "SAVE OUR CORALS BEFORE IT'S TOO LATE!!!",
    x=0.5,
    y=0.5,
